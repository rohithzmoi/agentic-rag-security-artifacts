...
We further illustrate these failure modes using a public reference implementation (AegisRAG), which demonstrates both direct and indirect leakage under post-filtered retrieval. We emphasize that this prototype is illustrative and not intended as a benchmark. Its purpose is to concretize failure modes discussed in Sections~\ref{sec:failure-taxonomy} and~\ref{sec:defense-taxonomy}.
...
\textbf{Illustrative demonstration reproducibility.} The screenshots in Figures~\ref{fig:direct} and~\ref{fig:indirect} were captured using the AegisRAG reference implementation. The artifact includes: (a) exact prompt text for each query, (b) role configuration (Junior HR), (c) mode selection (Vanilla RAG, Retrieve+Filter, Strict AFR), (d) expected outputs for each mode, and (e) raw screenshots. These materials allow reviewers to verify the failure mode demonstrations.
